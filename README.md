...
- Deployment on AWS  
  AWS repository host is configured in the CI/CD variable `AWS_REGISTRY_HOST`. This variable is inherited from 'Web Intelligence Hub Platform' . 
...
- In case of "Check Sum error" in the JHipster after editing an existing entity  
  error in the log :
...
or execute this query in Postgres DB :
...
To build the final jar and optimize the dataAcquisitionService application for production, run:
...
> **Prerequisites:**
>
> 1.  Use Powershell or any command line tool that supports bash scripts
> 2.  Set `vm.max_map_count` to at least `262144` For windows:
>
>         wsl -d docker-desktop
>         sysctl -w vm.max_map_count=262144
>         exit
>
>     [Guide](https://www.elastic.co/guide/en/elasticsearch/reference/current/docker.html#docker-prod-prerequisites)
>
> 3.  (Optional) Stop any other ELK instances
> 4.  Run `docker run -p 5601:5601 -p 9200:9200 -p 5044:5044 -it --name elk sebp/elk:7.10.0`
...
```
curl --location --request POST 'http://keycloak:9080/auth/realms/jhipster/protocol/openid-connect/token' \
--header 'Content-Type: application/x-www-form-urlencoded' \
--data-urlencode 'client_id=internal' \
--data-urlencode 'grant_type=client_credentials' \
--data-urlencode 'client_secret=internal'
```

## Upgrade JHipster

Update latest npm. If you face any issue or have nvm installed the use updateNpm.bat
...
We use the OWASP dependency check (maven plugin) to detect any potential vulnerabilities.\
The vulnerability scale is 0-10 and 7 is for high (fix in the next sprints) and 9-10 are critical (immediate fix).\
The plugin is configured to fail if a vulnerability is 7 or higher.\
The first time it takes around 20 minutes because it is downloading the vulnerability catalogs.\
Run the following command to execute:\
` ./mvnw verify -DskipTests=true -Pdependency`
Clean cache:\
`./mvnw dependency-check:purge -DskipTests=true -Pdependency`
