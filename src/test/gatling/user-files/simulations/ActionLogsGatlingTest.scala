...
/**
 * Performance test for the ActionLogs entity.
 */
class ActionLogsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ActionLogs entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all actionLogs")
            .get("/services/dataacquisitionservice/api/action-logs")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new actionLogs")
            .post("/services/dataacquisitionservice/api/action-logs")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "title":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_actionLogs_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created actionLogs")
                .get("/services/dataacquisitionservice${new_actionLogs_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created actionLogs")
            .delete("/services/dataacquisitionservice${new_actionLogs_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
