...
/**
 * Performance test for the Actions entity.
 */
class ActionsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Actions entity")
        ...
        .repeat(2) {
            ...
            .get("/services/dataacquisitionservice/api/actions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new actions")
            .post("/services/dataacquisitionservice/api/actions")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_actions_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created actions")
                .get("/services/dataacquisitionservice${new_actions_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created actions")
            .delete("/services/dataacquisitionservice${new_actions_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
