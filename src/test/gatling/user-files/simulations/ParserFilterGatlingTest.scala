...
/**
 * Performance test for the ParserFilter entity.
 */
class ParserFilterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ParserFilter entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all parserFilters")
            .get("/services/dataacquisitionservice/api/parser-filters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new parserFilter")
            .post("/services/dataacquisitionservice/api/parser-filters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_parserFilter_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created parserFilter")
                .get("/services/dataacquisitionservice${new_parserFilter_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created parserFilter")
            .delete("/services/dataacquisitionservice${new_parserFilter_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
