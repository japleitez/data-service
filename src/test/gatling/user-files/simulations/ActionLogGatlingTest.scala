...
/**
 * Performance test for the ActionLog entity.
 */
class ActionLogGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ActionLog entity")
        ...
        .repeat(2) {
            ...
            .exec(http("Create new actionLog")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_actionLog_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created actionLog")
                .get("/services/dataacquisitionservice${new_actionLog_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created actionLog")
            .delete("/services/dataacquisitionservice${new_actionLog_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
