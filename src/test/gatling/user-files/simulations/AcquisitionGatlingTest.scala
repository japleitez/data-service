...
/**
 * Performance test for the Acquisition entity.
 */
class AcquisitionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Acquisition entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all acquisitions")
            .get("/services/dataacquisitionservice/api/acquisitions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new acquisition")
            .post("/services/dataacquisitionservice/api/acquisitions")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "workflowId":null
                , "creawlerName":"SAMPLE_TEXT"
                , "startData":"2020-01-01T00:00:00.000Z"
                , "lastUpdateDate":"2020-01-01T00:00:00.000Z"
                , "status":"PROVISIONING"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_acquisition_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created acquisition")
                .get("/services/dataacquisitionservice${new_acquisition_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created acquisition")
            .delete("/services/dataacquisitionservice${new_acquisition_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
